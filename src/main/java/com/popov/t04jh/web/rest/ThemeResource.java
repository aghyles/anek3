...
/**
 * REST controller for managing {@link com.popov.t04jh.domain.Theme}.
 */
@RestController
@RequestMapping("/api")
@Transactional
public class ThemeResource {
     ...
     */
    @GetMapping("/themes")
    public ResponseEntity<List<Theme>> getAllThemes(Pageable pageable, @RequestParam(required = false, defaultValue = "false") boolean eagerload) {
        log.debug("REST request to get a page of Themes");
        Page<Theme> page;
        if (eagerload) {
            page = themeRepository.findAllWithEagerRelationships(pageable);
        } else {
            page = themeRepository.findAll(pageable);
        }
