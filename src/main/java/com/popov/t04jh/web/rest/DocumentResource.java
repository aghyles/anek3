...
/**
 * REST controller for managing {@link com.popov.t04jh.domain.Document}.
 */
@RestController
@RequestMapping("/api")
public class DocumentResource {
    ...
    public ResponseEntity<List<Document>> getAllDocuments(Pageable pageable, @RequestParam(required = false) String filter) {
        if ("testperformance-is-null".equals(filter)) {
            log.debug("REST request to get all Documents where testperformance is null");
            return new ResponseEntity<>(documentService.findAllWhereTestperformanceIsNull(),
                    HttpStatus.OK);
        }
