...
public interface ThemeRepository extends JpaRepository<Theme, Long> {
    ...
    @Query(value = "select distinct theme from Theme theme left join fetch theme.objectifs",
        countQuery = "select count(distinct theme) from Theme theme")
    Page<Theme> findAllWithEagerRelationships(Pageable pageable);

    @Query("select distinct theme from Theme theme left join fetch theme.objectifs")
    List<Theme> findAllWithEagerRelationships();

    @Query("select theme from Theme theme left join fetch theme.objectifs where theme.id =:id")
