...
export class ExerciceResolve implements Resolve<IExercice> {
  ...
  resolve(route: ActivatedRouteSnapshot): Observable<IExercice> {
    const id = route.params['id'];
    if (id) {
      return this.service.find(id).pipe(map((exercice: HttpResponse<Exercice>) => exercice.body));
    }
...
export const exerciceRoute: Routes = [
  {
    ...
    data: {
      ...
      defaultSort: 'id,asc',
      pageTitle: 't04JhApp.exercice.home.title'
    },
