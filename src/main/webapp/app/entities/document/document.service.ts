...
export class DocumentService {
  ...
  protected convertDateFromClient(document: IDocument): IDocument {
    const copy: IDocument = Object.assign({}, document, {
      dateDocument: document.dateDocument != null && document.dateDocument.isValid() ? document.dateDocument.toJSON() : null
    });
    return copy;
  }
  ...
  protected convertDateFromServer(res: EntityResponseType): EntityResponseType {
    if (res.body) {
      res.body.dateDocument = res.body.dateDocument != null ? moment(res.body.dateDocument) : null;
    }
    return res;
  }
  ...
  protected convertDateArrayFromServer(res: EntityArrayResponseType): EntityArrayResponseType {
    if (res.body) {
      res.body.forEach((document: IDocument) => {
        document.dateDocument = document.dateDocument != null ? moment(document.dateDocument) : null;
      });
    }
    return res;
  }
