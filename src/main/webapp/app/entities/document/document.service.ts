...
export class DocumentService {
  ...
  protected convertDateFromClient(document: IDocument): IDocument {
    const copy: IDocument = Object.assign({}, document, {
      dateDoc: document.dateDoc != null && document.dateDoc.isValid() ? document.dateDoc.toJSON() : null
    });
    return copy;
  }
  ...
  protected convertDateFromServer(res: EntityResponseType): EntityResponseType {
    if (res.body) {
      res.body.dateDoc = res.body.dateDoc != null ? moment(res.body.dateDoc) : null;
    }
    return res;
  }
  ...
  protected convertDateArrayFromServer(res: EntityArrayResponseType): EntityArrayResponseType {
    if (res.body) {
      res.body.forEach((document: IDocument) => {
        document.dateDoc = document.dateDoc != null ? moment(document.dateDoc) : null;
      });
    }
    return res;
  }
