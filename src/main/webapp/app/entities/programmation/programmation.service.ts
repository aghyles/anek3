...
export class ProgrammationService {
  ...
  create(programmation: IProgrammation): Observable<EntityResponseType> {
    return this.http.post<IProgrammation>(this.resourceUrl, programmation, { observe: 'response' });
  }

  update(programmation: IProgrammation): Observable<EntityResponseType> {
    return this.http.put<IProgrammation>(this.resourceUrl, programmation, { observe: 'response' });
  }

  find(id: number): Observable<EntityResponseType> {
    return this.http.get<IProgrammation>(`${this.resourceUrl}/${id}`, { observe: 'response' });
  }

  query(req?: any): Observable<EntityArrayResponseType> {
    const options = createRequestOption(req);
    return this.http.get<IProgrammation[]>(this.resourceUrl, { params: options, observe: 'response' });
  }
